...
function AuditRow({ task, onConfirm, canConfirm }: { task: CommunicationTask; onConfirm: () => void; canConfirm: boolean }) {
  return (
    <div className={styles.auditRow}>
      ...
      <Button size="small" disabled={task.confirmStatus === '已确认' || !canConfirm} onClick={onConfirm}>{task.confirmStatus === '已确认' ? task.confirmTime : '模拟确认'}</Button>
    </div>
  );
}
...
export default function Communication() {
  ...
  const { dashboard, communicationDispatches, communicationTasks, communicationLoading, communicationApiMode, loadCommunications, advanceCommunication, confirmCommunication } = useModel('qhse');
  const [eventId, setEventId] = useState('evt-001');
  useEffect(() => { void loadCommunications(); }, [loadCommunications]);

  const eventTasks = useMemo(() => communicationTasks.filter((task) => task.eventId === eventId), [communicationTasks, eventId]);
  if (communicationLoading && !communicationTasks.length) return <PageContainer><Skeleton active paragraph={{ rows: 14 }} /></PageContainer>;
  if (!communicationTasks.length) return <PageContainer><Empty description="通信数据暂不可用" /></PageContainer>;

  const events = communicationApiMode
    ? communicationDispatches.map((item) => ({ id: item.eventId, code: item.eventCode, title: item.eventTitle, areaName: item.areaName, level: item.eventLevel }))
    : (dashboard?.alarms ?? []).filter((event) => communicationTasks.some((task) => task.eventId === event.id));
  const activeEvent = events.find((event) => event.id === eventId) ?? events[0];
  const tasks = activeEvent?.id === eventId ? eventTasks : communicationTasks.filter((task) => task.eventId === activeEvent?.id);
  ...
  const delivered = communicationTasks.filter((task) => task.deliveryStatus === '已送达').length;
  const confirmed = communicationTasks.filter((task) => task.confirmStatus === '已确认').length;

  const escalate = async () => {
    if (!activeEvent) return;
    await advanceCommunication(activeEvent.id);
    message.warning(`已推进至 ${stageMeta[Math.min(currentStage + 1, 3)].title}`);
  };

  return (
    <PageContainer title={false} className={styles.page} extra={<Button danger disabled={currentStage >= 3 || hasConfirmed || !access.canSendCommunication} icon={<ClockCircleFilled />} onClick={() => void escalate()}>模拟无人确认并推进时间</Button>}>
      <section className={styles.hero}>
        <div><span>FUSED COMMUNICATION CONSOLE</span><h1>融合通信控制台</h1><p>统一调度 App、电话、短信与广播，完整记录送达、确认和升级过程。</p></div>
        <div className={styles.metrics}>
          <div><NotificationFilled /><strong>{communicationTasks.length}</strong><span>通信任务</span></div>
          ...
          <div className={styles.metricDanger}><AlertFilled /><strong>{communicationTasks.length - confirmed}</strong><span>待确认</span></div>
        </div>
      </section>
      ...
      <section className={styles.workspace}>
        <aside className={styles.eventQueue}>
          ...
          {events.map((event) => {
            const eventTaskList = communicationTasks.filter((task) => task.eventId === event.id);
        ...
        <article className={styles.escalationPanel}>
          ...
          <div className={styles.actions}><Button icon={<ReloadOutlined />} disabled={currentStage >= 3 || hasConfirmed || !access.canSendCommunication} onClick={() => void escalate()}>推进下一升级节点</Button><Button type="primary" disabled={!latestUnconfirmed || hasConfirmed || !access.canConfirmCommunication} onClick={() => latestUnconfirmed && void confirmCommunication(latestUnconfirmed.id)}>模拟责任人确认</Button></div>
        </article>

        <aside className={styles.channels}>
          <header><span>CHANNEL HEALTH</span><h2>通信渠道</h2></header>
          {[
            ['App消息', <AppstoreFilled key="app" />, '在线', '98.6%'],
            ['电话语音', <PhoneFilled key="phone" />, '在线', '96.2%'],
            ['短信', <MessageFilled key="message" />, '在线', '99.1%'],
            ['IP广播', <SoundFilled key="broadcast" />, '在线', '100%'],
      ...
      <section className={styles.audit}>
        ...
        {tasks.map((task) => <AuditRow key={task.id} task={task} canConfirm={access.canConfirmCommunication} onConfirm={() => void confirmCommunication(task.id)} />)}
      </section>
    </PageContainer>
  );
}
