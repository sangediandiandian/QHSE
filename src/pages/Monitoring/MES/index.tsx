...
export default function MesMonitoring() {
  const { dashboard, loading, loadDashboard, simulateJointAlarm } = useModel('qhse');
  const [step, setStep] = useState('全部');
  useEffect(() => { if (!dashboard) void loadDashboard(); }, [dashboard, loadDashboard]);

  const tags = useMemo(() => (dashboard?.mesTags ?? []).filter((tag) => step === '全部' || tag.processStep === step), [dashboard, step]);
  if (!dashboard && loading) return <PageContainer><Skeleton active paragraph={{ rows: 14 }} /></PageContainer>;
  if (!dashboard) return <PageContainer><Empty description="MES 数据暂不可用" /></PageContainer>;

  const cdu = dashboard.mesUnits[0];
  const anomalies = dashboard.mesTags.filter((tag) => tag.status === 'alarm').length;
  const warnings = dashboard.mesTags.filter((tag) => tag.status === 'warning').length;
  ...
  return (
    <PageContainer title={false} className={styles.page} extra={<Button danger icon={<ThunderboltFilled />} onClick={handleSimulation}>模拟 GDS + MES 联合预警</Button>}>
      <section className={styles.hero}>
        <div><span>MANUFACTURING EXECUTION SYSTEM</span><h1>MES 工艺数据关联</h1><p>生产负荷、设备参数与安全监测按装置和工艺节点统一关联。</p></div>
        <div className={styles.metrics}>
          <div><ApiOutlined /><strong>4/4</strong><span>在线装置</span></div>
      ...
      <section className={styles.unitStrip}>
        {dashboard.mesUnits.map((unit) => <article key={unit.id} className={styles[unit.status]}><span>{unit.code}</span><strong>{unit.name}</strong><em>{unit.load}% 负荷</em><small><i />{unit.operatingMode}</small></article>)}
      </section>
      ...
      <section className={styles.processPanel}>
        ...
        <div className={styles.processLine}>
          {steps.map((processStep, index) => {
            const stepTags = dashboard.mesTags.filter((tag) => tag.processStep === processStep);
      ...
      <section className={styles.correlation}>
        <article className={styles.gdsEvidence}>
          ...
          <div className={styles.evidenceValue}><span>GDS-101</span><strong>{gdsPoint?.currentValue ?? '--'}<em>%LEL</em></strong><Tag bordered={false}>{gdsPoint?.alarmStatus === 'level1' ? '一级报警' : '运行正常'}</Tag></div>
          <div className={styles.evidenceLink}><span><FireFilled /> 泵区可燃气体</span><i>+</i><span><DashboardFilled /> 出口压力/流量</span><i>=</i><strong>{jointEvent ? '疑似介质泄漏' : '未触发联合规则'}</strong></div>
          {jointEvent && <Button block danger onClick={() => history.push(`/warnings/${jointEvent.id}`)}>查看重大联合预警</Button>}
        </article>
        <article className={styles.ruleCard}><header><span>RULE · GDS_MES_01</span><Tag color={jointEvent ? 'error' : 'success'}>{jointEvent ? '已触发' : '监测中'}</Tag></header><h3>工艺介质泄漏联合研判</h3><p>泵出口压力升高或流量异常下降，同时附近 GDS 浓度持续上升，自动提升为重大预警。</p><div><span><CheckCircleFilled /> 空间距离 ≤ 50m</span><span><CheckCircleFilled /> 时间窗口 ≤ 5min</span><span><CheckCircleFilled /> 两类信号同时成立</span></div></article>
      </section>
