import type { WorkPermit as WorkPermitType, WorkPermitApplyInput, WorkPermitSiteConfirmation, WorkPermitStatus } from '@/types/qhse';
...
function PermitDetail({ permit, gdsPoints, canApprove, canConfirm, canControl, mutating, onAdvance, onApprove, onConfirm }: { permit: WorkPermitType; gdsPoints: Parameters<typeof calculatePermitNearestGdsDistance>[1]; canApprove: boolean; canConfirm: boolean; canControl: boolean; mutating: boolean; onAdvance: () => void; onApprove: () => void; onConfirm: (role: WorkPermitSiteConfirmation['role']) => void }) {
  ...
  return <section className={styles.detail}>
    ...
    {actionable && <Button block danger={permit.status === '建议暂停'} type="primary" loading={mutating} disabled={!canControl} icon={permit.status === '建议暂停' ? <PauseCircleFilled /> : <ExperimentFilled />} onClick={onAdvance}>{permit.status === '建议暂停' ? '确认暂停作业' : '复测合格并恢复作业'}</Button>}
  </section>;
}
...
export default function WorkPermit() {
  ...
  const [createForm] = Form.useForm<WorkPermitApplyInput>();
  const selectedAreaId = Form.useWatch('areaId', createForm);

  useEffect(() => { void loadWorkPermits(); }, [loadWorkPermits]);

  const permits = useMemo(() => permitRecords.filter((permit) => status === '全部' || permit.status === status), [permitRecords, status]);
  const selected = permitRecords.find((permit) => permit.id === selectedId) ?? permits[0];

  if (workPermitLoading && !permitRecords.length) return <PageContainer><Skeleton active paragraph={{ rows: 14 }} /></PageContainer>;

  const operating = permitRecords.filter((item) => item.status === '作业中').length;
  const highRisk = permitRecords.filter((item) => ['较大', '重大'].includes(item.riskLevel) && item.status !== '已关闭').length;
  const linkedAreas = new Set(workPermitAlarms.filter((alarm) => ['high', 'critical'].includes(alarm.level)).map((alarm) => alarm.areaId));
  const candidates = permitRecords.filter((permit) => permit.status === '作业中' && linkedAreas.has(permit.areaId)).length;
  ...
  return (
    <PageContainer title={false} className={styles.page} extra={<Space><Button icon={<FileAddFilled />} disabled={!access.canApplyPermit} onClick={() => { createForm.setFieldsValue({ type: '动火作业', riskLevel: '较大', startAt: '2026-07-15 09:00', endAt: '2026-07-15 17:00', gasTest: '可燃气体 0%LEL，氧含量 20.9%VOL', linkedGdsCodes: [], safetyMeasures: [], workX: 50, workY: 50 }); setCreateOpen(true); }}>申请作业票</Button><Button type="primary" icon={<ThunderboltFilled />} disabled={!linkageEnabled || !access.canControlPermit} onClick={runLinkage}>运行告警联动检查</Button></Space>}>
      ...
      <section className={styles.metrics}>
        <div><SafetyCertificateFilled /><span>今日票证<strong>{permitRecords.length}</strong><small>全部作业类型</small></span></div>
      ...
      <main className={styles.layout}>
        ...
        {selected && <PermitDetail permit={selected} gdsPoints={workPermitGdsPoints} canApprove={access.canApprovePermit} canConfirm={access.canConfirmPermit} canControl={access.canControlPermit} mutating={mutating} onApprove={async () => { setMutating(true); try { await approveWorkPermit(selected.id); message.success('当前审批节点已签署'); } finally { setMutating(false); } }} onConfirm={async (role) => { setMutating(true); try { await confirmWorkPermitSite(selected.id, role); message.success(`${role}已完成现场确认`); } finally { setMutating(false); } }} onAdvance={async () => {
          setMutating(true);
          try { await advanceWorkPermit(selected.id); message.success(selected.status === '建议暂停' ? '已确认暂停，现场作业应立即停止' : '气体复测合格，已恢复作业'); } finally { setMutating(false); }
        }} />}
      </main>

      <Modal width={720} title="申请作业票" open={createOpen} confirmLoading={mutating} onCancel={() => setCreateOpen(false)} onOk={() => createForm.validateFields().then(async (values) => { setMutating(true); try { await addWorkPermit(values); setCreateOpen(false); createForm.resetFields(); message.success('作业票已提交，进入三级审批'); } finally { setMutating(false); } })} okText="提交申请">
        <Form form={createForm} layout="vertical"><div className={styles.formGrid}><Form.Item name="type" label="作业类型" rules={[{ required: true }]}><Select options={['动火作业', '受限空间', '高处作业', '吊装作业', '临时用电'].map((value) => ({ value }))} /></Form.Item><Form.Item name="areaId" label="作业区域" rules={[{ required: true }]}><Select options={workPermitAreas.map((item) => ({ value: item.id, label: item.name }))} /></Form.Item><Form.Item name="guardian" label="现场监护人" rules={[{ required: true }]}><Input /></Form.Item><Form.Item name="startAt" label="计划开始" rules={[{ required: true }]}><Input /></Form.Item><Form.Item name="endAt" label="计划结束" rules={[{ required: true }]}><Input /></Form.Item><Form.Item name="riskLevel" label="作业风险" rules={[{ required: true }]}><Select options={['一般', '较大', '重大'].map((value) => ({ value }))} /></Form.Item><Form.Item name="linkedGdsCodes" label="关联 GDS 探测器"><Select mode="multiple" options={workPermitGdsPoints.filter((item) => !selectedAreaId || item.areaId === selectedAreaId).map((item) => ({ value: item.code, label: `${item.code} / ${item.name}` }))} /></Form.Item><Form.Item name="workX" label="作业点 X 坐标" rules={[{ required: true }]}><InputNumber min={0} max={100} /></Form.Item><Form.Item name="workY" label="作业点 Y 坐标" rules={[{ required: true }]}><InputNumber min={0} max={100} /></Form.Item></div><Form.Item name="workContent" label="作业内容" rules={[{ required: true }]}><Input.TextArea rows={2} /></Form.Item><Form.Item name="gasTest" label="气体检测结果" rules={[{ required: true }]}><Input /></Form.Item><Form.Item name="safetyMeasures" label="安全措施" rules={[{ required: true }]}><Select mode="tags" placeholder="输入措施后回车" /></Form.Item><p className={styles.formHint}>申请人由当前登录账号记录；坐标将在后续 GIS 接入阶段改为平面图点选。</p></Form>
      </Modal>
    </PageContainer>
  );
}
