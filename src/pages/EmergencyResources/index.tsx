...
import { getActiveEmergencyResourceDispatch } from '@/utils/emergencyResourceWorkflow';
import {
  CarFilled,
  CheckCircleFilled,
  ClockCircleOutlined,
...
export default function EmergencyResources() {
  ...
  const ready = allResources.filter((item) => item.availableQuantity > 0 && item.inspectionStatus !== '需要维护').length;
  ...
  return (
    <PageContainer title={false} className={styles.page} extra={<Button type="primary" icon={<CarFilled />} onClick={() => { resourceForm.resetFields(); setCreateOpen(true); }}>新增资源</Button>}>
      <header className={styles.heading}>
        <div><span>EMERGENCY RESOURCE CONTROL</span><h1>应急资源管理</h1><p>统一维护库存、检查状态、调拨占用、到位确认和归还记录。</p></div>
      ...
      <section className={styles.metrics} aria-label="资源战备指标">
        ...
        <div className={maintenance ? styles.warningMetric : ''}><span>检查关注</span><strong>{maintenance}<em>项</em></strong><small><WarningFilled /> 即将到期或需要维护</small></div>
      </section>
      ...
      <main className={styles.resourceLayout}>
          ...
          <div className={styles.resourceGrid}>{resources.map((resource) => {
            ...
            return (
              <article key={resource.id} className={`${styles.resourceCard} ${styles[resource.status]}`}>
                ...
                <div className={styles.quantity}>{resource.availableQuantity}<span>/ {resource.totalQuantity} {resource.unit} 可用</span></div>
                <dl>
                ...
                <footer>
                  ...
                  <Space size={4} wrap>
                    ...
                    {!activeDispatch && <Button size="small" type="primary" disabled={!resource.availableQuantity || resource.inspectionStatus === '需要维护'} onClick={() => openDispatch(resource)}>调拨</Button>}
        ...
        <aside className={styles.dispatchBoard}>
          ...
          <div className={styles.coverage}><span>资源战备覆盖</span><strong>{readyRate}%</strong><Progress percent={readyRate} showInfo={false} strokeColor="#1a7791" /><small>按有可用库存且无需维护的资源计算</small></div>
      ...
      <Modal title="新增应急资源" open={createOpen} okText="登记入库" cancelText="取消" onOk={() => void submitResource()} onCancel={() => setCreateOpen(false)} width={720} destroyOnClose>
        <Form form={resourceForm} layout="vertical" className={styles.formGrid} initialValues={{ type: '物资', totalQuantity: 1, unit: '套', eta: '5 分钟' }}>
      ...
      <Modal title={`调拨资源${dispatchResource ? ` · ${dispatchResource.name}` : ''}`} open={Boolean(dispatchResource)} okText="确认调拨" cancelText="取消" onOk={() => void submitDispatch()} onCancel={() => setDispatchResourceId(undefined)} destroyOnClose>
        <Form form={dispatchForm} layout="vertical">
          ...
          <Form.Item name="quantity" label={`调拨数量${dispatchResource ? `（可用 ${dispatchResource.availableQuantity} ${dispatchResource.unit}）` : ''}`} rules={[{ required: true }]}><InputNumber min={1} max={dispatchResource?.availableQuantity} precision={0} style={{ width: '100%' }} /></Form.Item>
      ...
      <Modal title={`${historyResource?.code ?? ''} 操作记录`} open={Boolean(historyResource)} footer={null} onCancel={() => setHistoryResourceId(undefined)} width={700} destroyOnClose>
        {historyResource && <div className={styles.historyPanel}>
          <section><h3>调拨与归还</h3>{historyResource.dispatches.length ? [...historyResource.dispatches].reverse().map((item) => <article key={item.id}><Tag color={item.status === '已归还' ? 'default' : item.status === '已到位' ? 'success' : 'processing'}>{item.status}</Tag><div><strong>{item.eventName} · {item.quantity} {historyResource.unit}</strong><p>{item.destination} · {item.operator}</p><small>调拨 {item.dispatchedAt}{item.arrivedAt ? ` · 到位 ${item.arrivedAt}` : ''}{item.returnedAt ? ` · 归还 ${item.returnedAt}` : ''}</small></div></article>) : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="暂无调拨记录" />}</section>
