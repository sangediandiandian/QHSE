...
export default function WarningRules() {
  ...
  return (
    <PageContainer title={false} className={styles.page} extra={<Popconfirm title="恢复 Mock 初始状态？" description="将清除浏览器中保存的告警、隐患、票证、规则和事件操作。" okText="确认重置" cancelText="取消" onConfirm={() => { void resetDashboard(); message.success('演示数据已恢复初始状态'); }}><Button icon={<ReloadOutlined />}>重置演示数据</Button></Popconfirm>}>
