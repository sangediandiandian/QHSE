...
function RuleItem({ rule, active, onSelect, onToggle }: {
  ...
  return <article className={`${active ? styles.active : ''} ${!rule.enabled ? styles.disabled : ''}`}>
    ...
    <Switch aria-label={`切换 ${rule.code}`} size="small" checked={rule.enabled} disabled={rule.version === 0} onChange={onToggle} checkedChildren="启" unCheckedChildren="停" />
  </article>;
}

export default function WarningRules() {
  const {
    dashboard, loading, loadDashboard, toggleWarningRule, resetDashboard,
  ...
  const pending = dashboard.warningRules.filter((rule) => rule.publishStatus === '待审批').length;
  ...
  return (
    <PageContainer title={false} className={styles.page} extra={<Space><Button type="primary" icon={<PlusOutlined />} onClick={() => openEditor()}>新建规则</Button><Popconfirm title="恢复 Mock 初始状态？" description="将清除浏览器中保存的风险评估、隐患证据、票证审批、规则会签、预案、事件审批和附件证据。" okText="确认重置" cancelText="取消" onConfirm={() => { void resetDashboard(); message.success('演示数据已恢复初始状态'); }}><Button icon={<ReloadOutlined />}>重置演示数据</Button></Popconfirm></Space>}>
