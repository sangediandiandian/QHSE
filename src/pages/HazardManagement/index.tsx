...
export default function HazardManagement() {
  ...
  return (
    <PageContainer title={false} className={styles.page}>
      ...
      <Modal title="上报隐患" open={createOpen} confirmLoading={mutating} onCancel={() => setCreateOpen(false)} onOk={() => createForm.validateFields().then(async (values) => { setMutating(true); try { const created = await addHazard(values); setCreateOpen(false); createForm.resetFields(); message.success(created?.recurrenceCount ? `隐患已上报，识别 ${created.recurrenceCount} 条历史同类隐患` : '隐患已上报并生成整改任务'); } finally { setMutating(false); } })} okText="上报并派发">
