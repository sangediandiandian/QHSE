...
export default function HazardManagement() {
  ...
  const { hazards: hazardRecords, hazardRiskUnits, hazardLoading, loadHazards, addHazard, addHazardEvidence, startHazard, submitHazard, acceptHazard, toggleHazardSupervision } = useModel('qhse');
  ...
  return (
    <PageContainer title={false} className={styles.page}>
      ...
      <Modal title={`添加整改证据 · ${selected?.code ?? ''}`} open={evidenceOpen} confirmLoading={mutating} onCancel={() => setEvidenceOpen(false)} onOk={() => evidenceForm.validateFields().then(async (values) => { if (!selected) return; setMutating(true); try { await addHazardEvidence(selected.id, values); setEvidenceOpen(false); evidenceForm.resetFields(); message.success('整改证据已归档'); } finally { setMutating(false); } })} okText="归档证据">
        <Form form={evidenceForm} layout="vertical"><Form.Item name="name" label="证据名称" rules={[{ required: true }]}><Input placeholder="例：整改后现场照片-01" /></Form.Item><Form.Item name="category" label="证据阶段" rules={[{ required: true }]}><Select options={['整改前', '整改过程', '整改完成'].map((value) => ({ value }))} /></Form.Item><Form.Item name="note" label="证据说明" rules={[{ required: true }]}><Input.TextArea rows={3} /></Form.Item><p className={styles.formHint}>上传人由当前登录账号自动记录；文件对象存储将在后续阶段接入。</p></Form>
      </Modal>
