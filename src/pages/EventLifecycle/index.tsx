...
function EventActions({ event, canManage, onAction }: { event: EmergencyEvent; canManage: boolean; onAction: (action: EmergencyEventAction) => void }) {
  ...
  return <div className={styles.actions}>{actions.map((action) => <Button
    ...
    danger={action === '终止响应'}
    disabled={!canManage || !isEmergencyEventActionAllowed(event, action)}
...
export default function EventLifecycle() {
  ...
  const { emergencyEvents, emergencyEventLoading, emergencyEventApiMode, loadEmergencyEvents, transitionEvent, addEmergencyEventEvidence, remindEmergencyClosureApproval, approveEmergencyEventClosure } = useModel('qhse');
  ...
  useEffect(() => { void loadEmergencyEvents(); }, [loadEmergencyEvents]);

  const events = useMemo(() => emergencyEvents.filter((event) => status === '全部' || event.status === status), [emergencyEvents, status]);
  const selected = emergencyEvents.find((event) => event.id === selectedId) ?? events[0];

  if (emergencyEventLoading && !emergencyEvents.length) return <PageContainer><Skeleton active paragraph={{ rows: 14 }} /></PageContainer>;
  if (!emergencyEvents.length) return <PageContainer><Empty description="事件生命周期数据暂不可用" /></PageContainer>;

  const responding = emergencyEvents.filter((event) => event.status === '响应中').length;
  const monitoring = emergencyEvents.filter((event) => event.status === '监控中' || event.status === '待关闭').length;
  const closed = emergencyEvents.filter((event) => event.status === '已关闭').length;
  const overdueApprovals = emergencyEvents.filter((event) => isEmergencyApprovalOverdue(event, getCurrentTimestamp())).length;
  const handleAction = async (action: EmergencyEventAction) => {
    ...
    await transitionEvent(selected.id, action);
  ...
  return <PageContainer title={false} className={styles.page}>
    <header className={styles.heading}>
      ...
      <div className={styles.guard}><SafetyCertificateFilled /><span>状态迁移受控<strong>{emergencyEventApiMode ? '服务端审计持久化' : '操作记录本地持久化'}</strong><small>非法状态不允许执行</small></span></div>
    </header>

    <section className={styles.metrics}>
      <div><AuditOutlined /><span>事件总数<strong>{emergencyEvents.length}</strong><small>统一事件台账</small></span></div>
    ...
    <section className={styles.toolbar}><Segmented value={status} onChange={(value) => setStatus(String(value))} options={['全部', ...statusOrder]} /><span>显示 {events.length} / {emergencyEvents.length} 个事件</span></section>
    ...
    <main className={styles.layout}>
      ...
      {selected && <section className={styles.detail}>
        <header><div><code>{selected.code}</code><h2>{selected.title}</h2><p>{selected.summary}</p></div><Space><Button icon={<PaperClipOutlined />} disabled={!access.canAddEmergencyEvidence || selected.status === '已关闭'} onClick={() => setEvidenceOpen(true)}>添加证据</Button><Tag color={statusColor[selected.status]}>{selected.status}</Tag></Space></header>
