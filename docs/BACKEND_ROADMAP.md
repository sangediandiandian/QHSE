...
IAM 管理接口只允许 `iam:manage` 账号更新已有用户的状态、所属组织、角色和区域授权，使用版本号阻止并发覆盖，并禁止管理员停用自身或移除自身管理员角色。区域角色必须分配至少一个有效区域，全局角色自动清空冗余区域。认证服务每次校验会话时读取当前实例的授权快照，因此单实例内角色变更即时生效，停用账号的已有会话立即失效。仓储支持默认进程内模式和 `QHSE_REPOSITORY=prisma` PostgreSQL 模式；Prisma 启动时加载组织、区域、角色和用户关系，授权更新在同一事务内递增 `tokenVersion` 并替换角色与区域关系。用户新增、自定义角色、统一身份源、权限变更审批和多实例授权变更广播安排在后续阶段。
