...
仓储支持默认进程内模式和 `QHSE_REPOSITORY=prisma` PostgreSQL 模式；Prisma 启动时加载组织、区域、角色、用户和审批台账，用户、密码、角色、授权关系和审批记录写入持久化存储。本阶段新增 `iam_authorization_requests` 表和审批状态枚举，目标环境必须在启用 Prisma 模式前执行受控 schema 同步或生成正式迁移。统一身份源和跨实例授权快照广播安排在后续阶段。
