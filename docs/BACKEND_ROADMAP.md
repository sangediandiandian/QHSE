...
- `GET /api/v1/auth/oidc/config|start|callback`：查询企业登录配置、启动 OIDC 授权码 + PKCE 流程并校验身份源回调。
- `POST /api/v1/auth/oidc/exchange`：使用短时一次性码兑换 QHSE 本地会话，浏览器地址中不传递访问令牌。
...
- `GET /api/v1/iam/authorization-requests`：查询授权变更申请与审批台账。
- `POST /api/v1/iam/users/:id/authorization-requests`：按用户版本提交状态、组织、角色和区域授权变更申请。
- `PUT /api/v1/iam/authorization-requests/:id/review`：由异于申请人的管理员批准或驳回，批准时原子应用授权。
- `PUT /api/v1/iam/users/:id/authorization`：兼容性直接授权接口；管理页面的标准路径使用双人审批。
- `GET /api/v1/audit-logs`：操作、登录与安全拒绝审计查询。
- `GET /api/v1/risks`：风险列表，支持 `areaId`、`level`、`keyword`。
- `GET /api/v1/risks/:id`：风险详情。
- `POST /api/v1/risks/:id/assessments`：提交待审批 LEC 评估，不直接改变当前风险等级。
- `PUT /api/v1/risks/:id/assessments/:assessmentId/review`：由异于评估人的授权账号批准或驳回，批准后风险等级生效。
...
- `POST /api/v1/hazards/duplicates/check`：按风险单元、类别和标题相似度检查历史同类隐患。
...
除健康检查、登录和 Swagger 外，接口默认需要 Bearer Token。演示账号包括 `admin`、`leader`、`qhse`、`dispatcher`、`unit_manager`、`operator`、`environment` 和 `commander`，本地演示密码统一为 `ant.design`。会话默认保存在进程内存中保证本地开箱即启，生产可切换 Redis。

企业登录支持 OIDC 授权码 + PKCE。启用时必须配置 `QHSE_OIDC_ENABLED=true`、`QHSE_OIDC_ISSUER`、`QHSE_OIDC_CLIENT_ID`、`QHSE_OIDC_REDIRECT_URI` 和 `QHSE_WEB_LOGIN_URL`；机密客户端再通过密钥管理系统注入 `QHSE_OIDC_CLIENT_SECRET`。`QHSE_OIDC_USERNAME_CLAIM` 默认读取 `preferred_username`，生产应改为客户身份源中不可变且能唯一匹配本地 IAM 账号的声明。多实例部署设置 `QHSE_OIDC_STORE=redis` 并提供 `QHSE_OIDC_REDIS_URL`，也可复用会话或平台 Redis 地址。完成真实身份源联调后可设置 `QHSE_LOCAL_LOGIN_ENABLED=false` 关闭本地密码入口；系统会拒绝在 OIDC 未启用时同时关闭本地登录，避免管理员被锁在系统外。
...
附件服务以统一对象元数据隔离业务表和二进制内容。上传时执行 MIME 白名单、大小限制、区域权限和 SHA-256 计算；对象只能绑定一条业务记录，同一绑定保持幂等。开发环境默认使用本地文件适配器，生产可切换 S3 兼容存储。隐患整改与应急事件证据已接入真实上传、业务绑定和鉴权下载；隐患移动端页面使用原生 `capture=environment` 调用后置摄像头，JPEG、PNG、WebP 照片仍进入同一受控附件链路。
...
IAM 管理接口只允许 `iam:manage` 账号创建用户、重置密码、创建/更新自定义角色或维护已有用户的状态、所属组织、角色和区域授权。新用户账号执行唯一性校验，初始密码使用每用户随机盐的 scrypt 摘要，列表和接口不返回密码摘要；认证仍兼容升级前的固定盐摘要。新账号和被重置密码的账号登录后只能查询本人、修改密码或退出登录，访问其他业务接口稳定返回 `PASSWORD_CHANGE_REQUIRED`；改密成功后需要使用新密码重新登录。会话保存密码摘要的 SHA-256 凭据版本指纹，密码变化后旧令牌即使尚未从会话存储物理清理也无法通过认证。内置角色保持只读，自定义角色编码创建后不可修改，可维护名称、权限点和全企业/授权区域数据范围；权限调整立即作用于现有会话，已分配用户时禁止切换数据范围类型，避免产生无区域授权账号。

管理页面调整用户授权时先固化目标用户版本、拟变更授权、申请人和原因，同一目标只允许存在一条待审批申请。申请人不能审批本人申请，驳回必须填写意见；批准时再次校验目标用户版本，并在同一 Prisma 事务中更新申请状态、用户版本、角色关系和区域关系，任一冲突整体回滚。待审批唯一活动键阻止多实例并发重复提交，授权生效后现有会话立即读取新权限。兼容性直接授权接口暂时保留，生产接入方应以审批接口作为标准路径。

仓储支持默认进程内模式和 `QHSE_REPOSITORY=prisma` PostgreSQL 模式；Prisma 启动时加载组织、区域、角色、用户和审批台账，用户、密码、角色、授权关系和审批记录写入持久化存储。`iam_authorization_requests` 表和审批状态枚举必须在启用 Prisma 模式前完成受控 schema 同步或正式迁移。

多实例部署可启用 Redis IAM 变更总线。用户创建、授权、密码、角色和审批写入成功后发布轻量事件，其他实例收到后从 PostgreSQL 重载完整 IAM 权威快照；本实例事件使用实例 ID 去重，连续远端事件按顺序刷新。Redis 订阅启动失败会阻止实例接流，运行中发布失败只标记降级，不把已经提交的数据库事务伪装成失败；订阅重连后主动全量校准，补偿中断期间遗漏的 Pub/Sub 消息。就绪探针和运行诊断均展示 IAM 广播后端、状态、发送、接收、重连校准与失败计数。

企业统一身份认证已形成 OIDC 基础闭环。服务启动时执行身份源发现，授权请求强制使用 PKCE S256、state 和 nonce，回调校验 ID Token 后只把指定声明映射到已存在且启用的 IAM 账号，不自动创建或提权。浏览器回调只携带 60 秒有效的一次性完成码，真实 QHSE 令牌由登录页调用兑换接口取得；事务 cookie 为 HttpOnly、SameSite=Lax，并限制在回调路径。身份源登录产生独立审计记录，OIDC 会话仍实时读取本地角色、区域和账号状态。生产剩余工作是客户 IdP 注册与声明治理，以及按需建设 JIT/SCIM、组映射和单点退出。
...
多实例 IAM 权限实时一致性必须与 PostgreSQL 仓储组合启用：

```bash
QHSE_REPOSITORY=prisma \
QHSE_IAM_EVENTS=redis \
QHSE_IAM_REDIS_URL=rediss://iam-events.example.com:6379 npm run server:dev
```

未设置 `QHSE_IAM_EVENTS=redis` 时使用进程内空操作适配器，适合单实例开发。可省略 `QHSE_IAM_REDIS_URL` 并复用 `QHSE_REDIS_URL`；Redis 连接由部署平台注入且不得写入仓库。配置 Redis IAM 广播但未启用 Prisma 仓储或未提供 Redis 地址时，服务拒绝启动，避免形成看似多实例、实际各节点数据独立的错误部署。
