...
IAM 管理接口只允许 `iam:manage` 账号创建用户或更新已有用户的状态、所属组织、角色和区域授权。新用户账号执行唯一性校验，初始密码使用每用户随机盐的 scrypt 摘要，列表和接口不返回密码摘要；认证仍兼容升级前的固定盐摘要。授权更新使用版本号阻止并发覆盖，并禁止管理员停用自身或移除自身管理员角色。区域角色必须分配至少一个有效区域，全局角色自动清空冗余区域。认证服务每次校验会话时读取当前实例的授权快照，因此单实例内角色变更即时生效，停用账号的已有会话立即失效。仓储支持默认进程内模式和 `QHSE_REPOSITORY=prisma` PostgreSQL 模式；Prisma 启动时加载组织、区域、角色和用户关系，用户创建和授权关系写入使用数据库事务。首次登录强制改密、管理员重置密码、自定义角色、统一身份源、权限变更审批和多实例授权变更广播安排在后续阶段。
