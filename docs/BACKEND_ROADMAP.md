...
| 2 | 组织、用户、角色、数据权限与审计拦截器 | 所有真实写接口有身份、权限和审计上下文 | 首版已完成：查询、账号启停、角色/区域授权和实时会话生效可用 |
...
IAM 首版管理接口只允许 `iam:manage` 账号更新已有用户的状态、所属组织、角色和区域授权，使用版本号阻止并发覆盖，并禁止管理员停用自身或移除自身管理员角色。区域角色必须分配至少一个有效区域，全局角色自动清空冗余区域。认证服务每次校验会话时读取当前账号授权，因此角色变更即时生效，停用账号的已有会话立即失效。当前可写仓储为进程内实现，PostgreSQL IAM 仓储、用户新增和自定义角色安排在后续生产化阶段。
