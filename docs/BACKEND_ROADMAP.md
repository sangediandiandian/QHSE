...
多实例部署可启用 Redis IAM 变更总线。用户创建、授权、密码、角色和审批写入成功后发布轻量事件，其他实例收到后从 PostgreSQL 重载完整 IAM 权威快照；本实例事件使用实例 ID 去重，连续远端事件按顺序刷新。Redis 订阅启动失败会阻止实例接流，运行中发布失败只标记降级，不把已经提交的数据库事务伪装成失败；订阅重连后主动全量校准，补偿中断期间遗漏的 Pub/Sub 消息。就绪探针和运行诊断均展示 IAM 广播后端、状态、发送、接收、重连校准与失败计数。统一身份源安排在后续阶段。
