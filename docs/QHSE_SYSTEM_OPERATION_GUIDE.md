...
组织与权限管理用于统一查看账号、组织、角色权限和区域数据范围，并由系统管理员维护已有账号授权。它解决“谁能看、谁能办、能看哪些区域”的平台治理问题。
...
- 使用版本号防止两个管理员同时修改时相互覆盖。

### 20.3 操作方法

1. 使用 `admin` 登录，进入“权限管理”。
2. 搜索目标账号，核对当前组织、角色、数据范围和状态。
3. 点击“授权”。
4. 选择启用或停用、所属组织和一个或多个角色。
5. 若所选角色为区域数据权限，至少选择一个区域；全企业角色无需选择区域。
6. 点击“保存授权”，看到成功提示后核对版本号递增。
7. 被停用账号的现有会话立即失效；角色或区域调整也会立即作用于已有会话。

### 20.4 安全边界

- 只有具备 `iam:manage` 权限的账号能够保存授权。
- 当前管理员不能停用自身，也不能移除自己的系统管理员角色，避免把平台锁死。
- 页面只维护已有账号，不提供删除；用户新增、角色自定义和统一身份源接入属于后续生产化能力。
- 演示环境授权保存在进程内，后端重启后恢复种子授权；生产部署需接入 PostgreSQL IAM 仓储。

### 20.5 客户讲解话术

> 平台权限不是只控制菜单显示，后端会对按钮操作和区域数据再次校验。授权变更立即作用于现有会话，停用账号无需等待会话自然过期，同时通过审计和版本号保证变更可追溯、并发修改不互相覆盖。
