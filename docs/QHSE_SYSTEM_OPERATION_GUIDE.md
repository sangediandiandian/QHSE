...
API 模式下，应急事件关闭后会自动建立复盘档案；调查结论、调查附件、整改计划维护、隐患转单及状态同步、可信操作人、区域权限、乐观锁和关闭归档均由服务端执行。转单同时要求 `emergency:manage` 与 `hazard:report`，风险单元与事件区域不一致时服务端拒绝创建。当前尚未接入企业知识库检索和正式复盘报告导出，这些能力仍需项目化完善。
