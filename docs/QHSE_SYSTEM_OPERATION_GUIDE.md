...
6. 持续推进直到全部改进项完成。
7. 使用具备 `emergency:approve` 权限的 QHSE 账号完成关闭并归档复盘。

### 17.4 当前边界

API 模式下，应急事件关闭后会自动建立复盘档案；调查结论、调查附件、整改计划维护、可信操作人、区域权限、乐观锁和关闭归档均由服务端执行。当前尚未接入整改任务与隐患领域联动、企业知识库检索和正式复盘报告导出，这些能力仍需项目化完善。
