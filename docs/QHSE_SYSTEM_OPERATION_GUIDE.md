...
3. 查看并讲解直接原因、管理根因及经验教训。
4. 为每个改进项明确优先级、责任人和状态。
5. 持续推进直到全部改进项完成。
6. 使用具备 `emergency:approve` 权限的 QHSE 账号完成关闭并归档复盘。

### 17.4 当前边界

API 模式下，复盘台账、整改推进、可信操作人、区域权限、乐观锁和关闭归档均由服务端执行。当前尚未接入事件关闭后自动建档、结构化调查报告编辑、调查附件、整改任务与隐患领域联动、企业知识库检索和正式复盘报告导出，这些能力仍需项目化完善。
