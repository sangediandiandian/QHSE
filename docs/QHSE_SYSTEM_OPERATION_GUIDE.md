...
现场演示应实事求是。当前版本中，风险、隐患、作业许可、监测接入、规则、预警、应急事件、预案、资源、通信、报表、平台配置和运行诊断已有前后端业务能力。以下内容属于展示、模拟或待项目化集成能力：
...
- “事件复盘”在当前 API 模式下可能没有数据，完整服务端复盘工作流仍需继续建设。
...
3. 填写或讲解直接原因、管理根因及经验教训。
4. 为每个改进项明确优先级、责任人和状态。
5. 持续推进直到全部改进项完成。
6. 完成后关闭并归档复盘。

### 17.4 当前边界

当前页面属于基本展示和交互版本。在 API 模式下可能没有复盘数据，完整的服务端复盘工作流、附件和整改联动仍是后续建设内容。正式客户演示前应先确认页面中存在可讲解数据；若为空，可说明该能力的设计目标，不要宣称已完成生产闭环。
