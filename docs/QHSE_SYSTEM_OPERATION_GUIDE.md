...
现场演示应实事求是。当前版本中，风险、隐患、作业许可、监测接入、规则、预警、应急事件、事件复盘、预案、资源、通信、报表、平台配置和运行诊断已有前后端业务能力。以下内容属于展示、模拟或待项目化集成能力：
...
| -------------- | ------------------- | ------------ | ------------------------------------ |
| `admin`        | 系统管理员          | 全部区域     | 平台配置、运行诊断、跨角色兜底操作   |
...
2. 输入演示账号和统一密码。
3. 登录成功后进入综合驾驶舱。
4. 当前会话有效期为 8 小时，同一账号保留最近 5 个会话。
5. 若按钮置灰，通常表示当前账号没有该操作权限；若列表数据较少，可能是该账号只允许查看所属区域。
...
> 一个风险点同时保留固有风险和当前动态风险。比如原本是一般风险，但同区域出现气体报警、工艺异常或高风险作业时，当前风险可以被动态提升。系统把监测信号和管控措施放在同一档案中，帮助管理人员解释“为什么升级”和“应该采取什么措施”。

### 6.7 注意事项

- 当前版本的风险评估提交后直接形成评估记录，没有独立的风险评估审批页面。
- 风险等级不能只看颜色，应结合 LEC 分值、动态因素和现场实际判断。
- 没有“风险评估”或“维护措施”权限的账号只能查看。
...
2. 点击“上传证据”，选择证据阶段、说明和文件。
3. 至少上传一项证据后，提交验收。
4. 验收人员查看隐患内容、整改措施和附件。
5. 填写验收意见并确认，状态变为“已关闭”。
...
> 隐患不是填一条记录就结束。系统要求明确责任和期限，用现场证据证明整改过程，再由有权限的人员验收关闭。重大或逾期隐患还可以挂牌督办，整个过程保留操作者和时间记录。
...
12. 使用具备 `report:export` 权限的账号点击“导出复盘报告”，下载包含调查结论、时间线、整改措施、隐患编号和证据哈希目录的 HTML 报告。
13. 在浏览器打开报告后可直接打印，选择“另存为 PDF”即可形成便于对外归档的 PDF 版本；未归档报告会显示“待归档”水印。

### 17.4 当前边界

API 模式下，应急事件关闭后会自动建立复盘档案；调查结论、调查附件、整改计划维护、隐患转单及状态同步、正式报告导出、可信操作人、区域权限、乐观锁和关闭归档均由服务端执行。转单同时要求 `emergency:manage` 与 `hazard:report`，风险单元与事件区域不一致时服务端拒绝创建；报告导出同时要求 `emergency:read` 与 `report:export`。当前尚未接入企业知识库检索。
...
4. 查看耗时较高或错误较多的路由。
5. 对比更新时间，确认页面仍在自动刷新。
6. 出现异常时记录节点、路由、时间和追踪标识，再交给运维人员排查。
...
| 29–30 分钟 | 平台配置、诊断           | 可配置、可集成、可运维       | `admin`                  |
